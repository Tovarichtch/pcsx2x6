...
#define Ra3 (iopMemReadString(a3))
	...
	namespace ioman
	{
		...
		int open_HLE()
		{
			...
			// S246 LLE: real firmware opens rom1:/mc0:/ac0:/HST: — serve from game directory
			if (!hostRoot.empty() && (path.starts_with("mc0:") || path.starts_with("rom1:") || path.starts_with("ac0:") || path.starts_with("HST:")))
			{
				...
				if (name.starts_with("/"))
					name = name.substr(1);
				std::string local = Path::Combine(hostRoot, name + ".img");
				if (!FileSystem::FileExists(local.c_str()))
					local = Path::Combine(hostRoot, name);

				if (FileSystem::FileExists(local.c_str()))
				{
					if (!freefdcount()) { v0 = -IOP_EMFILE; pc = ra; return 1; }
					const int hostfd = FileSystem::OpenFDFile(local.c_str(), O_RDONLY | O_BINARY, 0);
					if (hostfd < 0) { v0 = -IOP_ENOENT; pc = ra; return 1; }
					file = new HostFile(hostfd);
					v0 = allocfd(file);
					if ((s32)v0 < 0)
						file->close();
					else {
						fileHandle h; h.fd_index = v0 - firstfd; h.flags = flags; h.full_path = path; h.mode = mode;
						handles.push_back(h);
					}
					pc = ra;
					return 1;
