...
static std::string s_acgame_mcd_override[2];
static bool s_acgame_usbgun = false;
...
void VMManager::UpdateDiscDetails(bool booting)
{
	...
	for (int x = 0; x < 2; x++)
	{
		if (!s_acgame_mcd_override[x].empty())
		{
			int indx = FileMcd_ConvertToSlot(x, 0);
			EmuConfig.Mcd[indx].Filename = s_acgame_mcd_override[x];
	...
	if (!filename.empty())
	{
		...
		else if (isArcadeManifest(filename))
		{
			...
			if (!INI.Load()){
				...
				s_acgame_usbgun = INI.GetBoolValue("data", "usbgun", false);
...
VMBootResult VMManager::Initialize(const VMBootParameters& boot_params, Error* error)
{
	...
	if (s_acgame_usbgun)
	{
		ACJV::CurrentBoardID = TSS_GUN_EXTENTION;
		Console.WriteLn(Color_Green, "ACGAME: JVS gun board enabled (TSS-I/O GUN-EXTENTION)");
	}
